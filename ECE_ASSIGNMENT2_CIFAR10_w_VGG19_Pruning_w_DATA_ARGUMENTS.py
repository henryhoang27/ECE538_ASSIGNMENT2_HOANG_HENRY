# ================================================================
# File: pruned_vgg19_cifar10_manual_compare_augmented.py
# Description: Train a VGG19 model on CIFAR-10 with manual pruning,
#              fine-tuning, and performance comparison.
# ================================================================

import os
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Disable oneDNN optimizations for deterministic results

import numpy as np
import time
import tensorflow as tf
from tensorflow.keras.applications import VGG19
from tensorflow.keras.layers import Input, Dense, GlobalAveragePooling2D
from tensorflow.keras.models import Model
from tensorflow.keras import mixed_precision
from sklearn.model_selection import train_test_split
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix, classification_report
import matplotlib.pyplot as plt
import seaborn as sns

# ================================================================
# -------------------- CONFIGURATION -----------------------------
# ================================================================

mixed_precision.set_global_policy('float32')   # Use full precision to avoid numeric instability
BATCH_SIZE = 32
EPOCHS = 15                  # Initial training epochs (unpruned model)
FINE_TUNE_EPOCHS = 15       # Fine-tuning epochs after pruning
FINAL_SPARSITY = 0.4        # Target sparsity for manual pruning (40%)
NUM_CLASSES = 10            # CIFAR-10 has 10 classes
PLOT_DIR = "plots"          # Directory for saving plots
os.makedirs(PLOT_DIR, exist_ok=True)

# ================================================================
# -------------------- LOAD & PREPARE DATA -----------------------
# ================================================================

# Load CIFAR-10 dataset (50k train, 10k test)
(x_train, y_train), (x_test, y_test) = tf.keras.datasets.cifar10.load_data()

# Normalize pixel values to [0, 1]
x_train = x_train.astype("float32") / 255.0
x_test  = x_test.astype("float32") / 255.0

# Resize images from (32x32) to (224x224) to match VGG19 input requirements
x_train = tf.image.resize(x_train, (224, 224)).numpy()
x_test  = tf.image.resize(x_test, (224, 224)).numpy()

# One-hot encode labels for multi-class classification
y_train = tf.keras.utils.to_categorical(y_train, NUM_CLASSES)
y_test  = tf.keras.utils.to_categorical(y_test, NUM_CLASSES)

# Split training set into training and validation sets (80/20 split)
x_train, x_val, y_train, y_val = train_test_split(
    x_train, y_train, test_size=0.2, random_state=42
)

# ================================================================
# -------------------- DATA AUGMENTATION -------------------------
# ================================================================

# Define a sequential pipeline of random augmentations
data_augmentation = tf.keras.Sequential([
    tf.keras.layers.RandomFlip("horizontal"),   # Random horizontal flips
    tf.keras.layers.RandomRotation(0.1),        # Random small rotations
    tf.keras.layers.RandomZoom(0.1),            # Random zoom-in/zoom-out
    tf.keras.layers.RandomContrast(0.1)         # Random contrast adjustment
], name="data_augmentation")

AUTOTUNE = tf.data.AUTOTUNE  # Use autotuning for parallel data loading

# Training dataset (with augmentation)
train_data = (
    tf.data.Dataset.from_tensor_slices((x_train, y_train))  # Convert arrays to TensorFlow dataset
    .shuffle(10000)                                         # Shuffle data for randomness
    .batch(BATCH_SIZE)                                      # Batch the data
    .map(lambda x, y: (data_augmentation(x, training=True), y), 
         num_parallel_calls=AUTOTUNE)                       # Apply augmentations on-the-fly
    .cache()                                                # Cache for performance
    .prefetch(AUTOTUNE)                                     # Overlap preprocessing and model execution
)

# Validation and test datasets (no augmentation)
val_data = (
    tf.data.Dataset.from_tensor_slices((x_val, y_val))
    .batch(BATCH_SIZE)
    .cache()
    .prefetch(AUTOTUNE)
)

test_data = (
    tf.data.Dataset.from_tensor_slices((x_test, y_test))
    .batch(BATCH_SIZE)
    .cache()
    .prefetch(AUTOTUNE)
)

# ================================================================
# -------------------- MODEL BUILDER -----------------------------
# ================================================================

def build_model(num_classes=NUM_CLASSES):
    """
    Builds a VGG19-based classification model for CIFAR-10.
    The VGG19 backbone is pre-trained on ImageNet and frozen.
    """
    inputs = Input(shape=(224, 224, 3), name="input_image")

    # Optional augmentation layer inside the model (redundant but enhances robustness)
    x = data_augmentation(inputs)

    # Load VGG19 without top layers (pretrained on ImageNet)
    backbone = VGG19(include_top=False, weights="imagenet", input_tensor=x)
    backbone.trainable = False  # Freeze convolutional base

    # Global Average Pooling replaces dense top to reduce parameters
    x = GlobalAveragePooling2D(name="gap")(backbone.output)

    # Dense classifier head
    x = Dense(256, activation="relu", dtype="float32", name="fc1")(x)
    outputs = Dense(num_classes, activation="softmax", dtype="float32", name="predictions")(x)

    return Model(inputs=inputs, outputs=outputs, name="vgg19_unpruned")

# ================================================================
# -------------------- MANUAL MAGNITUDE PRUNING ------------------
# ================================================================

def magnitude_prune_model(model, final_sparsity):
    """
    Manually prune the model by zeroing out the smallest-magnitude weights.
    Args:
        model: trained Keras model
        final_sparsity: proportion of weights to prune per layer
    Returns:
        The pruned model
    """
    total_params = 0
    pruned_params = 0

    print("\n--- Layer-wise Pruning Report ---")
    for layer in model.layers:
        # Prune only Dense and Conv2D layers
        if isinstance(layer, (tf.keras.layers.Dense, tf.keras.layers.Conv2D)):
            weights = layer.get_weights()
            if not weights:
                continue

            kernel = weights[0]  # Only prune kernel, not bias
            flat = np.abs(kernel).reshape(-1)
            n_total = flat.size

            # Determine pruning threshold based on final_sparsity ratio
            k = int(np.floor(final_sparsity * n_total))
            if k <= 0:
                continue

            thresh = np.partition(flat, k)[k]  # Find cutoff magnitude
            mask = np.abs(kernel) > thresh      # Create binary mask
            kernel_pruned = kernel * mask       # Apply mask (zero out small weights)

            # Reconstruct new weights and set them back to the layer
            new_weights = [kernel_pruned]
            if len(weights) > 1:
                new_weights.append(weights[1])  # Retain bias

            try:
                layer.set_weights(new_weights)
            except Exception as ex:
                print(f"Warning: could not set weights for {layer.name}: {ex}")

            # Count pruned parameters for report
            layer_pruned = (n_total - mask.sum())
            total_params += n_total
            pruned_params += layer_pruned

            print(f"{layer.name:<20s} | size={n_total:8d} | pruned={layer_pruned:8d} "
                  f"| sparsity={(100.0*layer_pruned/n_total):5.2f}%")

    # Compute global sparsity across model
    global_sparsity = 100.0 * pruned_params / total_params
    print(f"\nManual pruning applied. Global sparsity: {global_sparsity:.2f}% "
          f"({pruned_params}/{total_params} params pruned)")
    return model

# ================================================================
# -------------------- SPARSITY ANALYSIS -------------------------
# ================================================================

def compute_model_sparsity(model, prefix=""):
    """
    Computes the percentage of zero weights in the model.
    """
    total = 0
    zeros = 0
    for layer in model.layers:
        if isinstance(layer, (tf.keras.layers.Dense, tf.keras.layers.Conv2D)):
            weights = layer.get_weights()
            if not weights: continue
            kernel = weights[0]
            total += kernel.size
            zeros += np.sum(kernel == 0)
    sparsity = 100.0 * zeros / total if total > 0 else 0.0
    print(f"{prefix} Model Sparsity: {sparsity:.2f}% "
          f"({zeros}/{total} weights are zero)")
    return sparsity

# ================================================================
# -------------------- EVALUATION & REPORTING --------------------
# ================================================================

def evaluate_and_report(model, test_data, y_test, prefix="unpruned"):
    """
    Evaluate the model on test data and print detailed metrics + confusion matrix.
    """
    start_time = time.time()
    y_pred_probs = model.predict(test_data)  # Forward pass
    end_time = time.time()

    # Convert probabilities to class indices
    y_pred_classes = np.argmax(y_pred_probs, axis=1)
    y_true_classes = np.argmax(y_test, axis=1).reshape(-1)

    # Compute standard classification metrics
    acc  = accuracy_score(y_true_classes, y_pred_classes)
    prec = precision_score(y_true_classes, y_pred_classes, average='weighted', zero_division=0)
    rec  = recall_score(y_true_classes, y_pred_classes, average='weighted', zero_division=0)
    f1   = f1_score(y_true_classes, y_pred_classes, average='weighted', zero_division=0)

    # Compute inference throughput and latency
    total_time = end_time - start_time
    num_samples = y_test.shape[0]
    throughput = num_samples / total_time if total_time > 0 else float('inf')
    latency = (total_time / num_samples) * 1000 if num_samples > 0 else 0.0

    print(f"\n==== {prefix.upper()} MODEL METRICS ====")
    print("Accuracy:", acc)
    print("Precision:", prec)
    print("Recall:", rec)
    print("F1 Score:", f1)
    print(f"Total Inference Time: {total_time:.2f} sec | Throughput: {throughput:.2f} samples/sec | Latency: {latency:.2f} ms")

    # Class-wise metrics and confusion matrix visualization
    class_names = ['airplane','automobile','bird','cat','deer','dog','frog','horse','ship','truck']
    print("\nPer-Class Metrics:")
    print(classification_report(y_true_classes, y_pred_classes, target_names=class_names, zero_division=0))

    # Plot confusion matrix
    cm = confusion_matrix(y_true_classes, y_pred_classes)
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=class_names, yticklabels=class_names)
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title(f'Confusion Matrix ({prefix})')
    plt.savefig(os.path.join(PLOT_DIR, f'confusion_matrix_{prefix}.png'))
    plt.close()

    return {"acc": acc, "prec": prec, "rec": rec, "f1": f1}

# ================================================================
# -------------------- TRAIN UNPRUNED MODEL ----------------------
# ================================================================

model = build_model(NUM_CLASSES)
model.compile(optimizer=tf.keras.optimizers.Adam(1e-4),
              loss='categorical_crossentropy',
              metrics=['accuracy'])

# Initial training on augmented dataset
history_unpruned = model.fit(train_data, validation_data=val_data, epochs=EPOCHS)

# Evaluate unpruned model on test set
metrics_unpruned = evaluate_and_report(model, test_data, y_test, prefix="unpruned")

print("\nPRE-pruned model summary:")
model.summary()
compute_model_sparsity(model, prefix="Unpruned")

# ================================================================
# -------------------- APPLY PRUNING + FINE-TUNE -----------------
# ================================================================

print("\nApplying manual magnitude pruning...")
model = magnitude_prune_model(model, FINAL_SPARSITY)

# Fine-tune pruned model with lower learning rate
model.compile(optimizer=tf.keras.optimizers.Adam(1e-5),
              loss='categorical_crossentropy',
              metrics=['accuracy'])
history_pruned = model.fit(train_data, validation_data=val_data, epochs=FINE_TUNE_EPOCHS)

# Evaluate pruned model
metrics_pruned = evaluate_and_report(model, test_data, y_test, prefix="pruned")

print("\nPOST-pruned model summary:")
model.summary()
compute_model_sparsity(model, prefix="Pruned")

# ================================================================
# -------------------- VISUALIZE PERFORMANCE ---------------------
# ================================================================

plt.figure(figsize=(12, 5))

# Accuracy plot before & after pruning
plt.subplot(1, 2, 1)
plt.plot(history_unpruned.history['accuracy'], label='Unpruned Train')
plt.plot(history_unpruned.history['val_accuracy'], label='Unpruned Val')
plt.plot([None]*EPOCHS + history_pruned.history['accuracy'], label='Pruned Train')
plt.plot([None]*EPOCHS + history_pruned.history['val_accuracy'], label='Pruned Val')
plt.title('Accuracy Before & After Pruning')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()

# Loss plot before & after pruning
plt.subplot(1, 2, 2)
plt.plot(history_unpruned.history['loss'], label='Unpruned Train')
plt.plot(history_unpruned.history['val_loss'], label='Unpruned Val')
plt.plot([None]*EPOCHS + history_pruned.history['loss'], label='Pruned Train')
plt.plot([None]*EPOCHS + history_pruned.history['val_loss'], label='Pruned Val')
plt.title('Loss Before & After Pruning')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()

plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, 'accuracy_loss_compare.png'))
plt.close()
